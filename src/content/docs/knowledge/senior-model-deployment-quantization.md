...
| 资料 | 关键结论 | 工程含义 |
|---|---|---|
| ONNX Concepts / IR | ONNX 用通用语言描述模型，生产环境围绕 runtime 执行 graph；ONNX 是强类型 IR，graph、node、tensor、opset 都有明确语义 | 部署不是保存模型文件，而是稳定表达 graph、operator、type、opset 和 runtime 兼容性 |
| ONNX Shape Inference | shape inference 不保证完整；动态行为会阻断推断，例如 Reshape 的 shape 来自 runtime tensor；不是所有 op 都有 shape inference 实现 | shape inference 是辅助检查，不是部署正确性证明 |
...
| ONNX Runtime Execution Providers | ORT 通过 EP 框架对接 CPU、GPU、NPU 等后端，provider 优先级会影响节点分配和 fallback | 硬件加速是 graph partition + backend capability + fallback，而不是“整图切到某个硬件” |
...
治理：模型、工具链、runtime、driver、硬件版本如何可复现？
```

高级工程师看部署链路时，不会只问“模型能不能跑”，而会问：
...
是否能定位出错位置？
是否能量化性能和精度风险？
是否有 fallback、版本漂移、数据前后处理不一致的问题？
```
...
```text
1. Source model
   PyTorch / TensorFlow / Caffe / JAX

2. Export / Convert
   Torch ONNX export / Caffe parser / custom converter

3. Intermediate representation
   ONNX / MLIR / TFLite / vendor IR

4. Validation
   checker / shape inference / CPU runtime / numerical compare

5. Graph optimization
   constant folding / dead node elimination / fusion / layout rewrite

6. Quantization / precision lowering
   FP32 -> FP16 / INT8 / INT4 / FP8

7. Backend compilation
   TensorRT engine / RKNN / OpenVINO IR / vendor blob

8. Runtime integration
   memory binding / input output / threading / streams / contexts

9. Production operation
   profiling / monitoring / rollback / model version governance
```

面试中如果只讲第 2 层和第 3 层，会显得非常浅。高级回答必须贯穿 validation、optimization、precision、backend、runtime 和 production。

## 3. ONNX：不是 runtime，而是强类型计算图交换格式
...
- 前后处理一致；
- 图优化正确性；
- 大模型 external data 管理；
- runtime provider 分配。

### 3.2 ONNX graph 的关键对象

| 对象 | 高级理解 | 常见工程问题 |
|---|---|---|
| ModelProto | 模型容器，包含 metadata、opset_import、graph | producer/opset 不记录会导致问题不可复现 |
| GraphProto | 计算图，不只是节点列表，还有输入输出和 initializer | 手工改图容易破坏拓扑或 SSA |
| NodeProto | 算子调用，包含 op_type、domain、inputs、outputs、attributes | node name 不可靠，逐层对齐不能只靠名字 |
| TensorProto | 张量数据，包括权重、常量、稀疏或外部数据 | 大权重 external data 路径易丢 |
| Initializer | 常量 tensor，通常是权重或 bias | 老模型可能把 initializer 也列为 graph input |
| ValueInfo | 类型和 shape 信息，是 shape inference 和调试的重要依据 | 中间 value_info 不完整时要运行 shape inference |
| Opset | 算子语义版本；同名 op 在不同版本下可能有不同定义 | runtime 支持范围、默认属性变化、降版本风险 |
...
如果要降 opset，是否会改变语义？
目标 NPU converter 支持哪个 opset 范围？
```

## 4. Shape、Layout、Dtype：部署正确性的三条主线

### 4.1 Shape

Shape 决定 tensor 大小、buffer plan、kernel selection 和 graph legality。

常见问题：

| 问题 | 结果 | 排查 |
|---|---|---|
| dynamic batch 未声明 | runtime 输入失败 | 检查 input shape 和 export dynamic_axes |
| Reshape 目标来自 runtime tensor | shape inference 无法完整推断 | 打印 value_info、固定输入复现 |
| Concat 维度不一致 | runtime 报错或 shape 错 | 对每个输入输出 shape 做表格 |
| NPU 只支持 static shape | converter 失败或 fallback | 固定 shape 或按尺寸分 engine |
| TensorRT profile 范围过宽 | latency 抖动或 tactic 不优 | 收窄 min/opt/max profile |

### 4.2 Layout

Layout 是同一个 tensor 内存如何解释的问题。图像模型里常见：
...
- 第一层 Conv 后就错，优先查 RGB/BGR、NCHW/NHWC、weight layout；
- 性能差但数值对，优先查 transpose/layout transform；
- NPU/GPU 后端常有内部 blocked layout，dump 中间 tensor 时要确认是否已经回到逻辑 layout；
- layout transform 可能消耗大量带宽，尤其是大 feature map。

### 4.3 Dtype

Dtype 不是简单的精度选择，它会影响：

- kernel 是否可用；
- accumulation dtype；
- quantization boundary；
- memory bandwidth；
- tensor core / NPU MAC 路径；
- CPU fallback；
- 输出数值容忍度。
...
|---|---|
| FP32 | 准确但慢、内存大 |
| FP16 | 溢出/下溢、LayerNorm/Softmax 可能敏感 |
| BF16 | 动态范围好，但硬件支持依赖平台 |
| INT8 | 需要 calibration 和 scale/zero_point |
| INT4 | 多用于 weight-only，依赖专门 kernel |
| INT64 | 很多 NPU 不支持，常用于 shape/index tensor |
...
### 5.3 Reshape / Transpose / Slice / Gather
...
| Gather | index dtype、动态 shape、axis 支持常出问题 |
| Concat | axis 错误导致张量语义错误 |
| Flatten | 起始 axis 不同导致 classifier 输入错 |
| Resize | 坐标变换、nearest/linear、align_corners 类语义差异 |
| Softmax | axis 错导致概率分布完全错误 |

## 6. MLIR：从格式转换到合法化 pipeline

### 6.1 为什么 MLIR 适合模型部署工具链
...
### 6.2 Operation / Value / Region / Block
...
### 6.3 Pattern Rewrite 和 Dialect Conversion 的区别
...
## 8. Dynamic Shape：编译期优化和运行时灵活性的 trade-off
...
- engine cache；
- latency 稳定性；
- NPU compiler 的 tiling 和 buffer plan。
...
- calibration 估计的是激活范围，不是模型训练。

### 9.2 QOperator vs QDQ
...
| Weight-only | 主要量化权重 | LLM 低 bit 压缩和访存优化 |

ORT 文档建议一般 RNN/Transformer 可优先 dynamic，CNN 可优先 static；如果 PTQ 达不到精度目标，再考虑 QAT。

### 9.4 INT4 / Weight-only

低 bit 量化不是简单把所有 tensor 变成 INT4。工程上要看：
...
- kernel 是否真的加速；
- 精度是否可接受；
- scale/zero_point 元数据开销是否抵消收益。

## 10. 逐层调试：高级工程师必须会做的事情
...
| PC 模拟快板端慢 | 带宽、驱动、温控、CPU 后处理、拷贝 |

## 12. 应用到你的项目经历

### 12.1 Caffe 到 ONNX / MLIR
...
15. 如果 target EP 只有部分节点支持，如何证明端到端仍然值得使用？
...
11. 对同一模型分别跑 CPU EP、CUDA/TensorRT/NPU EP，统计 fallback 和 per-node latency。
12. 写部署报告：正确性、性能、fallback、版本、风险、回滚方案。
